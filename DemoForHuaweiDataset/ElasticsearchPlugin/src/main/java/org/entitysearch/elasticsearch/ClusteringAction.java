...
public class ClusteringAction
    ...
    public static  ArrayList<String> category = new ArrayList<String>(); // for multiple category query
    ...
    public static class ClusteringActionResponse extends ActionResponse implements ToXContent {
        ...
        public XContentBuilder toXContent(XContentBuilder builder, Params params)
            ...
            for(SearchHit hit : hits) {
                String Cat = (String)hit.sourceAsMap().get("entityCategory");
                if (!categorySet.contains(Cat))
                {
                    continue;
                }
                HashMap<String, ArrayList<String>> hashMapCategory = new HashMap<String, ArrayList<String>> ();
                HashMap<Integer, Integer>beginEnd = new HashMap<Integer, Integer>();
                ...
                for(String k: hashMapCategory.keySet())
                {
                       ...
                       beginArr.add(begin);
                       beginEnd.put(begin, end);
                } 
                // sort begin 
                Collections.sort(beginArr);
                
                for(int b:beginArr) 
                {
                        StringBuffer sb=new StringBuffer();
                        int e = beginEnd.get(b);
                        for(i=b; i<=e; i++)
                        {
                            if(b==e)
