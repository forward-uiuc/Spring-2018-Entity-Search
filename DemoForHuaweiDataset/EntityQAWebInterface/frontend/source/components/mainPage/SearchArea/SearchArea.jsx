...
class SearchArea extends Component {
		...
		for(let i = 0; i  < currentEntityArr.length; i++){
			for(let j = 0; j < hits.length;j++){
				if(hits[j]._id == currentEntityArr[i].id){
					output.push(hits[j])
				    console.log("hit",hits[j])
												//get the post.txt name
					let value = hits[j]._source.physicalDoc;
				...
				for(let i = 0; i  < cluster.length; i++){
					for(let j = 0; j < hits.length;j++){
						if(hits[j]._id == cluster[i].document[0].id){
							...
							let value = hits[j]._source.physicalDoc;
							let fileName = value.substr(0,value.indexOf('.')-0);
							let fileUrl = this.baseUrl+ "getPhysicalDoc/" + value+".txt"
							axios.get(fileUrl)
								.then((response)=>{
									// console.log(response.data)
									let content = response.data
	...
	phyDocHandler(value,text,event){
		...
		let fileUrl = this.baseUrl+ "getPhysicalDoc/" + value +".txt" // change
		axios.get(fileUrl)
			.then((response)=>{
				//console.log(response.data)
				let Data = response.data
				var ques = Data.split('@')[0]
				var content = Data.split('@')[1]
				//var str = JSON.stringify(response);
				//console.log("cont: "+content)
				//console.log("ques: "+ques)
				//let Question = response.ques
 	...
 	handleClose(){
   	...
   	render(){
			...
			<Header content={currentDocNum} />
			    ...
			    <Modal.Content scrolling>
					<div>
						{Question}
						<br />
						<span className="highlightAns">{physicalDoc}</span>
   		...
   		let EntityResult =(
   			<div className="resultcard1">
				 	...
				 	<div className="header resultheader">
				 		<Header as='h2'>找到 {cluster.length} 条结果</Header>
				 	</div>
							...
							<div className="content">
								<div className="description resultText">
	 								<span>{clusterExamples[i]._source.text.substr(0,clusterExamples[i]._source.charOffset)}</span>
	 							 	...
	 							 	<span> <a onClick={this.phyDocHandler.bind(this,clusterExamples[i]._source.physicalDoc,clusterExamples[i]._source.text)}>更多...</a> </span>
	 							</div>
								<div className="meta">score:&nbsp;{clusterExamples[i]._score} (from {item.document.length} results) </div>
 					 			<div className="description resultText">
	 								<a onClick={this.ClusterHandler.bind(this,i,item.name)}> 更多相关结果</a>
	 							</div>
 							</div>
					 	...
					 	<div key={i} className="ui card padded clustercard raised">
							<div className="content">
								...
								<div className="description resultText">
	 								...
	 								<span><a onClick={this.phyDocHandler.bind(this,item._source.physicalDoc,item._source.text)}> 更多..</a></span>
	 							</div>
   			...
   			<Container className="searchbar">
				<Grid columns='equal'>
					...
					<Grid.Column width={7}>
						...
						<Dropdown text='搜索模式' floating labeled button className='icon'>
						    <Dropdown.Menu>
   		...
   		let suggestInput = (
   			...
   			<Container className="searchbar">
   					<Header size='small'>如何搜索</Header>
   				  	<Grid columns='equal'>
					    <Grid.Row className="example">
					      	<Grid.Column>
					       		<Header as='h4' 
					       				color='red'
					       				onClick={this.handleExampleQuery.bind(this,"#p #a #f 模糊")}>
					       				#p #a #f 模糊
					       		</Header> 
					     	</Grid.Column>
					      	<Grid.Column>
								<Header as='h4' color='orange' 
										onClick={this.handleExampleQuery.bind(this,"#a #s huawei nova 2 plus")}>
										#a #s huawei nova 2 plus
								</Header>  						       		
					      	</Grid.Column>
					      	<Grid.Column>
					       		<Header as='h4' 
					       				color='olive'
					       				onClick={this.handleExampleQuery.bind(this,"#h nova 2 plus")}>
					       				#h nova 2 plus
					       		</Header>
					      	</Grid.Column>
					    </Grid.Row>
					    ...
					    <Grid.Row>
						      <Grid.Column width={5}>
						      	 	<Header as='h4' 
						      	 	 		color='teal'
						      	 	 		onClick={this.handleExampleQuery.bind(this,"#a #p mate")}>
						      	 	 		#a #p mate
						      	 	</Header>
						      </Grid.Column>
						      <Grid.Column width={6}>
						       		<Header as='h4' 
						       				color='blue'
						       				onClick={this.handleExampleQuery.bind(this,"#p #s 卡顿")}>
						       				#p #s 卡顿
						       		</Header>
						      </Grid.Column>
						      <Grid.Column width={5}>
						       		<Header as='h4' 
						       				color='violet'
						       				onClick={this.handleExampleQuery.bind(this,"#a #f #p 手机")}>
						       				#a #f #p 手机
						       		</Header>
						      </Grid.Column>
					    	...
					    	searchbyItem === 'entity' ?
								...
								<Grid.Column width={16}>
									<div className="ui card resultcard">
										<div className="content ">
				 							<div className="header resultheader">
				 								<Header as='h2'>找到 {output.length} 文档</Header>
				 							</div>
				 							 {DocumentResult}
				 						</div>
				 					</div>	
								...
								<Grid columns={1}>						
						 			<Grid.Column width={16}>
						 				<div className="ui card resultcard1">
						 					<div className="content">
						 					 	<Header as='h3'>您所搜索的内容不存在</Header>
						 					 </div>
						 				</div>
						 			</Grid.Column>
							 	</Grid>
